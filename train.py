...
from criterion_new import CL_sentiment, CL_InfoNCE
...
from data_utils_new import Tokenizer4Bert, ABSAAugDataset, ABSADataset
...
class Instructor:
    def __init__(self, opt):
        ...
        self.trainset = ABSAAugDataset(opt.dataset_file['train'], tokenizer)
        print("Train size:", len(self.trainset))
        # self.valset is used for Twitter-15 and Twitter-17 dataset
        # self.valset = ABSADataset(opt.dataset_file['dev'], tokenizer)
        # print("Dev size:", len(self.valset))
        self.testset = ABSADataset(opt.dataset_file['test'], tokenizer)
        print("Test size:",len(self.testset))

        # we believe valset = testset for REST14,LAP14,and TWITTER14 datasets
    ...
    def _train(self, criterion, contrastiveLoss_sentiment, contrastiveloss_InfoNCE, optimizer, train_data_loader, val_data_loader):
        max_val_acc = 0
        max_val_acc_f1 = 0
        ...
        for i_epoch in range(self.opt.num_epoch):
            ...
            if "_aug" in self.opt.dataset_file['train']:
                for i_batch, batch in enumerate(train_data_loader):
                    ...
                    outputs = self.model(inputs_text)
                    inputs_augment = [batch['concat_text2_bert_indices'].to(self.opt.device),
                              batch['concat_text2_segments_indices'].to(self.opt.device)]
                    outputs_augment = self.model(inputs_augment)

                    targets = batch['polarity'].to(self.opt.device)
                    polabel = batch['polabel'].to(self.opt.device)
                    loss1 = criterion(outputs[0], targets)
                    loss2 = contrastiveLoss_sentiment(outputs[1], polabel)
                    loss3 = contrastiveloss_InfoNCE(outputs[1], outputs_augment[1])
    ...
    def run(self):
        ...
        contrastiveloss_InfoNCE = CL_InfoNCE(self.opt)
        ...
        best_model_path, max_val_acc, max_val_acc_f1, max_val_f1, max_val_f1_acc = self._train(
            criterion, contrastiveLoss_sentiment, contrastiveloss_InfoNCE, optimizer, train_data_loader, val_data_loader)
...
def main():
    ...
    parser.add_argument('--temperatureP', default=0.08, type=float)
    parser.add_argument('--temperatureY', default=0.08, type=float)
    ...
    dataset_files = {
        ...
        "cl_aug_sr_acl2014": {
            'train': './datasets/Aug_Data/Aug_acl14/twitter14_sr_aug.txt',
            'test': './datasets/acl-14-short-data/test.raw'
        },
        "cl_aug_sr_res2014": {
            'train': './datasets/Aug_Data/Aug_res14/res14_sr_aug.txt',
            'test': './datasets/semeval14/restaurant_test.raw'
        },
        "cl_aug_sr_laptop14": {
            'train': './datasets/Aug_Data/Aug_laptop14/laptop14_sr_aug.txt',
            'test': './datasets/semeval14/laptop_test.raw'
        },
        "cl_aug_bt_twitter15": {
            'train': './datasets/Aug_Data/Aug_twitter15/twitter15_bt_aug.txt',
            'dev': './datasets/Twitter15/twitter15_dev.txt',
            'test': './datasets/Twitter15/twitter15_test.txt'
        },
        "cl_aug_sr_twitter15": {
            'train': './datasets/Aug_Data/Aug_twitter15/twitter15_sr_aug.txt',
            'dev': './datasets/Twitter15/twitter15_dev.txt',
            'test': './datasets/Twitter15/twitter15_test.txt'
        },
        "cl_aug_bt_twitter17": {
            'train': './datasets/Aug_Data/Aug_twitter17/twitter17_bt_aug.txt',
            'dev': './datasets/Twitter17/twitter17_dev.txt',
            'test': './datasets/Twitter17/twitter17_test.txt'
        },
        "cl_aug_sr_twitter17": {
            'train': './datasets/Aug_Data/Aug_twitter17/twitter17_sr_aug.txt',
            'dev': './datasets/Twitter17/twitter17_dev.txt',
            'test': './datasets/Twitter17/twitter17_test.txt'
        }
